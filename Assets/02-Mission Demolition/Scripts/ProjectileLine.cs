...
public class ProjectileLine : MonoBehaviour
{
    ...
    public Vector3 lastPoint
    {
        get {
            if (points == null)
            {
                //If there are bi oiubts, returns Vector3.zero
                return (Vector3.zero);
            }
    ...
    void FixedUpdate()
    {
        if (poi == null)
        {
            ...
            //If there is a POI, it's loc is aded every FixedUpdate
            AddPoint();
            if(FollowCam.POI == null)
            {
                //Once FollowCam.POI is null, make the local poll null too
                poi = null;
            }
        }
    }
